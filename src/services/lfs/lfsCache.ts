...
export class LfsCache {
    ...
    constructor(
        ...
        try {
            if (!fs.existsSync(this.cacheDir)) {
                ...
                console.log('[LfsCache] Created cache directory:', this.cacheDir);
            }
        } catch (error) {
            console.error('[LfsCache] Failed to create cache directory:', error);
        }
    }
    ...
    public get(filePath: string, version: string): Buffer | undefined {
        ...
        if (memEntry && (Date.now() - memEntry.timestamp < this.maxAge)) {
            console.log('[LfsCache] Memory cache hit:', filePath);
        ...
        if (fs.existsSync(diskPath)) {
            ...
            if (age < this.maxAge) {
                console.log('[LfsCache] Disk cache hit:', filePath);
                ...
                try {
                    ...
                    console.error('[LfsCache] Failed to read cache file:', error);
                    // Delete corrupted cache file
                    try {
                        fs.unlinkSync(diskPath);
                    } catch (deleteError) {
                        console.error('[LfsCache] Failed to delete corrupted cache file:', deleteError);
                    }
                }
            } else {
                // Expired, delete it
                console.log('[LfsCache] Cache entry expired, deleting:', filePath);
                try {
                    fs.unlinkSync(diskPath);
                } catch (error) {
                    console.error('[LfsCache] Failed to delete expired cache file:', error);
                }
            }
        }
    ...
    public set(filePath: string, version: string, content: Buffer): void {
        ...
        console.log('[LfsCache] Caching file:', {
            filePath,
        ...
        try {
            ...
            console.error('[LfsCache] Failed to write cache file:', error);
    ...
    private cleanup(): void {
        try {
            ...
            console.log('[LfsCache] Cache size:', {
            ...
            if (totalSize > this.maxCacheSize) {
                console.log('[LfsCache] Cache size exceeded, performing cleanup...');
                ...
                for (const fileInfo of fileStats) {
                    ...
                    try {
                        ...
                        console.error('[LfsCache] Failed to delete cache file:', fileInfo.path, error);
                    }
                }

                console.log('[LfsCache] Cleanup complete:', {
            ...
            console.error('[LfsCache] Cleanup failed:', error);
        }
    }
     ...
     */
    public clear(): void {
        console.log('[LfsCache] Clearing all cache...');
        ...
        try {
            ...
            for (const file of files) {
                try {
                    ...
                    console.error('[LfsCache] Failed to delete cache file:', file, error);
                }
            }

            console.log('[LfsCache] Cache cleared:', {
                deletedFiles: deletedCount
            });
        } catch (error) {
            console.error('[LfsCache] Failed to clear cache:', error);
        }
    }
    ...
    public getStats(): { totalSizeMB: number; fileCount: number; maxSizeMB: number } {
        try {
            ...
            console.error('[LfsCache] Failed to get stats:', error);
            return {
                totalSizeMB: 0,
                fileCount: 0,
                maxSizeMB: this.maxCacheSize / (1024 * 1024)
